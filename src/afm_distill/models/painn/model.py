...
@final
class PaiNNCutoffFnConfig(C.Config):
    """Configuration for the cutoff function used in PaiNN."""
...
@final
class PaiNNRBFConfig(C.Config):
    """Configuration for the radial basis functions used in PaiNN."""
        ...
        match self.fn_type:
            ...
            case "bessel":
                if self.start is not None:
                    log.warning(
                        "Start parameter is ignored for bessel RBFs in PaiNN.")
                if self.trainable:
                    log.warning(
                        "Trainable parameter is ignored for bessel RBFs in PaiNN.")
                return BesselRBF(
                    n_rbf=self.n_rbf,
                    cutoff=cutoff,
                )
...
class PaiNNStudentModelConfig(StudentModuleBaseConfig):
    ...
    cutoff_fn: PaiNNCutoffFnConfig
    """Cutoff function configuration."""

    neighbor_list_fn: PaiNNNeighborListConfig = PaiNNNeighborListConfig()
    ...
    rbf: PaiNNRBFConfig = PaiNNRBFConfig()
    """Radial basis function configuration."""

    @override
    def create_model(self) -> PaiNNStudentModel:
        return PaiNNStudentModel(self)
    ...
    def ensure_dependencies(cls):
        if importlib.util.find_spec("schnetpack") is None:
            raise ImportError(
                "schnetpack is not installed. Please install schnetpack to use PaiNNModelConfig."
            )


@final
class PaiNNStudentModel(
    StudentModuleBase[dict[str, torch.Tensor],
                      dict[str, torch.Tensor], PaiNNStudentModelConfig]
):
    @override
    @classmethod
    def hparams_cls(cls):
        return PaiNNStudentModelConfig
