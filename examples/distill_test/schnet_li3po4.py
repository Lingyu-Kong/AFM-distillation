...
from afm_distill.models import SchNetStudentModel


def main(args_dict: dict):
    def hparams():
        hparams = OfflineDistillationConfig.draft()
        hparams.model = DC.SchNetStudentModelConfig.draft()
        hparams.model.cutoff = args_dict["cutoff"]
        hparams.model.cutoff_fn = DC.SchNetCutoffFnConfig(fn_type="cosine")
        hparams.model.neighbor_list_fn = DC.SchNetNeighborListConfig(
            fn_type="pymatgen")
        hparams.model.n_atom_basis = 30
        hparams.model.num_message_passing = args_dict["num_message_passing"]
        hparams.model.rbf = DC.SchNetRBFConfig(fn_type="gaussian")
        ...
        os.system(
            f"rm -rf ./checkpoints/schnet-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}.ckpt")
        hparams.trainer.checkpoint = MC.ModelCheckpointConfig(
            monitor="val/forces_rmse",
            dirpath="./checkpoints",
            filename=f"schnet-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}",
            save_top_k=1,
            mode="min",
        )
        now = datetime.now()
        ...
        hparams.trainer.loggers = [
            WandbLoggerConfig(
                ...
                name=f"schnet--{args_dict['cutoff']}A-T={args_dict['num_message_passing']}-li3po4-{formatted}"
            )
        ]
    ...
    model = SchNetStudentModel.load_from_checkpoint(
        f"./checkpoints/schnet-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}.ckpt")
    ...
    plt.tight_layout()
    plt.savefig("schnet_li3po4_val_predictions.png", dpi=300)
