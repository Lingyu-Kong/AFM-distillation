...
def main(args_dict: dict):
    def hparams():
        ...
        os.system(
            f"rm -rf ./checkpoints/painn_from_dft-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}.ckpt")
        hparams.trainer.checkpoint = MC.ModelCheckpointConfig(
            monitor="val/forces_rmse",
            dirpath="./checkpoints",
            filename=f"painn_from_dft-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}",
            save_top_k=1,
            mode="min",
        )
        now = datetime.now()
        ...
        hparams.trainer.loggers = [
            WandbLoggerConfig(
                ...
                name=f"painn_from_dft-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}-{formatted}"
            )
        ]
    ...
    model = PaiNNStudentModel.load_from_checkpoint(
        f"./checkpoints/painn_from_dft-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}.ckpt")
    ...
    plt.tight_layout()
    plt.savefig("painn_from_dft_val_predictions.png", dpi=300)
