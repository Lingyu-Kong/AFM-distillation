...
def main(args_dict: dict):
    def hparams():
        ...
        hparams.data.train.src = "/nethome/lkong88/MatterTune/examples/water-thermodynamics/data/train_water_870_eVAng.xyz"
        ...
        os.system(
            f"rm -rf ./checkpoints/allegro_from_dft-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}.ckpt")
        hparams.trainer.checkpoint = MC.ModelCheckpointConfig(
            monitor="val/forces_mae",
            dirpath="./checkpoints",
            filename=f"allegro_from_dft-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}",
            save_top_k=1,
            mode="min",
        )
        now = datetime.now()
        ...
        hparams.trainer.loggers = [
            WandbLoggerConfig(
                ...
                name=f"allegro-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}-{formatted}"
            )
        ]
    ...
    model = AllegroStudentModel.load_from_checkpoint(
        f"./checkpoints/allegro_from_dft-{args_dict['cutoff']}A-T={args_dict['num_message_passing']}.ckpt", weights_only=False)
    ...
    plt.tight_layout()
    plt.savefig("allegro_from_dft_val_predictions.png", dpi=300)
